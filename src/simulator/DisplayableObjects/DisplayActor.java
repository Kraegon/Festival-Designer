...
public class DisplayActor extends DisplayObject implements Serializable {
	...
	public Point2D getRandomPoint2D() {
		return (new Point2D.Double(lastTargetLocation.getX() - 75
				+ Math.random() * 150, lastTargetLocation.getY() - 75
				+ Math.random() * 150));
	}

	public void update(List<DisplayObject> otherObjects) {
		location = new Point2D.Double(location.getX() + speed
				* Math.cos(direction), location.getY() + speed
				* Math.sin(direction));
		...
		if (location.getX() > targetLocation.getX() - boundary
				&& location.getX() < targetLocation.getX() + boundary
				&& location.getY() > targetLocation.getY() - boundary
				&& location.getY() < targetLocation.getY() + boundary) {
			if (actions.get(0).hasNeighbour()) {
				addTarget(actions.get(0).getRandomNeighbour()); // add a
																// neighbour to
																// list
				actions.remove(0); // remove location from list
				setTargetLocation(actions.get(0).getLocation()); // set neighbor
																	// as new
																	// targetlocation
				lastTargetLocation = actions.get(actions.size() - 1)
						.getLocation();
			} else {
	...
	private boolean collisionObject(DisplayActor otherActor,
			DisplayObject otherObject) {
		...
		Rectangle2D.Double rectActor = new Rectangle2D.Double(xa, ya,
				(double) otherActor.getSize().getHeight(), (double) otherActor
						.getSize().getWidth());
		Rectangle2D.Double rectObject = new Rectangle2D.Double(xo, yo,
				(double) otherObject.getSize().getHeight(),
